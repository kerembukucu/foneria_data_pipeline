...
WITH fund_clicks AS (
    ...
    GROUP BY customer_id, fund_id, year, month
),
...
fund_outstanding AS (
    SELECT
        ...
        fund_id,
        SUM(fund_volume) AS fund_outstanding
...
SELECT
    ...
    COALESCE(fc.fund_id, ftv.fund_id, fo.fund_id) AS fund_id,
    fc.fund_clicks,
    fo.fund_outstanding,
...
FULL OUTER JOIN fund_outstanding fo
    ...
    AND COALESCE(fc.fund_id, ftv.fund_id) = fo.fund_id;
